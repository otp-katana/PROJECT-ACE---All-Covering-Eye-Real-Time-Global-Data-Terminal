
# ───────────────────────────────────────────────────────────────────────────
# ── PART: 1 ][ USGS PROVIDER ───────────────────────────────────────────────
# ───────────────────────────────────────────────────────────────────────────
# Fetches live earthquake data from USGS's "all_day" feed — all magnitudes,...
# ...refreshed roughly every 60 seconds on USGS's end (polling faster than...
# ...that on our side would just return the same cached data; see...
# ...MainLayout.jsx's 60s interval). Coordinates arrive as [lon, lat, depth]...
# ...per GeoJSON convention — index 2 (depth) is not used by the volcano...
# ...provider, since GVP has no equivalent field.
import httpx

from app.protocols.omori.schemas import SeismicEvent

USGS_URL = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.geojson"


async def fetch_usgs_events() -> list[SeismicEvent]:
    async with httpx.AsyncClient(timeout=10.0) as client:
        response = await client.get(USGS_URL)
        response.raise_for_status()
        data = response.json()

    events = []
    for feature in data.get("features", []):
        coords = feature["geometry"]["coordinates"]    # [lon, lat, depth]
        props = feature["properties"]
        events.append(
            SeismicEvent(
                lat=coords[1],
                lon=coords[0],
                mag=props.get("mag", 0.0),
                place=props.get("place"),
                time=str(props.get("time")),
                depth=coords[2] if len(coords) > 2 else None,
                updated=str(props.get("updated")) if props.get("updated") else None,
                felt=props.get("felt"),
                tsunami=props.get("tsunami"),
            )
        )
    return events
